...
/**
 * Ported verbatim from club-admin/components/RichTextEditor.tsx for support
 * ticket replies. Output is HTML, constrained to exactly the tag set the
 * backend sanitiser (common/rich-text.util.ts) allows: inline marks, lists,
 * h2-h4, blockquote, links. Anything the editor can't produce can't survive
 * a round-trip, so the two stay in lock-step — if you change the extensions
 * here, update the sanitizer's whitelist too, in both consoles.
 */
...
export function RichTextEditor({
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "drift-prose min-h-[120px] px-3 py-2.5 text-[13.5px] text-drift-text-primary focus:outline-none",
...
/** Read-only render of already-sanitised rich text. */
