...
import { DefinitionList, DetailRail, StatBand } from "@/components/dashboard-design";
...
export default function CommercialPaymentsPage() {
  ...
  const totals = rows?.totalsByCurrency.length
    ? rows.totalsByCurrency
    : [{ currency: "USD", collectedMinor: 0, refundedMinor: 0, failedMinor: 0, transactions: 0 }];
    ...
    <div>
      ...
      <StatBand
        stats={totals.map((total) => ({
          ...
          value: money(total.collectedMinor, total.currency),
          note: `${money(total.refundedMinor, total.currency)} refunded / ${money(total.failedMinor, total.currency)} failed`,
          icon: "payments",
          tone: total.failedMinor > 0 ? "amber" : "green",
        }))}
      />

      <Card className="mb-4 p-4">
        <div className="grid gap-3 lg:grid-cols-[minmax(220px,1fr)_160px_150px_120px]">
          <Field label="Search"><Input aria-label="Search transactions" placeholder="Search invoice, description, reference..." value={search} onChange={(event) => { setSearch(event.target.value); setRows(null); }} /></Field>
          <Field label="Status"><Select value={status} onChange={(event) => { setStatus(event.target.value as PaymentTransactionStatus | ""); setRows(null); }}><option value="">Any status</option><option value="PENDING">Pending</option><option value="SUCCEEDED">Succeeded</option><option value="FAILED">Failed</option><option value="REFUNDED">Refunded</option></Select></Field>
          <Field label="Audience"><Select value={audience} onChange={(event) => { setAudience(event.target.value as BillingAudience | ""); setRows(null); }}><option value="">Any audience</option><option value="CLUB">Club</option><option value="PLAYER">Player</option></Select></Field>
          <Field label="Currency"><Input aria-label="Currency" placeholder="KES" maxLength={3} value={currency} onChange={(event) => { setCurrency(event.target.value.toUpperCase()); setRows(null); }} /></Field>
        </div>
      </Card>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
              ...
              <table className="w-full min-w-[960px]">
                ...
                <tbody>
                  {rows.transactions.map((transaction) => (
                    <tr key={transaction.id} className={detail?.id === transaction.id ? "bg-drift-primary-light/60" : ""}>
                      <Td><div className="font-bold">{transaction.invoice.number}</div><div className="text-xs text-drift-text-secondary">{transaction.invoice.plan.name}</div></Td>
                      <Td><div>{transaction.owner.name}</div><div className="text-xs text-drift-text-secondary">{label(transaction.owner.type)}</div></Td>
                      <Td><span className="font-bold tabular">{money(transaction.amountMinor, transaction.currency)}</span></Td>
                      ...
                      <Td className="text-right"><button className="font-bold text-drift-primary hover:underline focus:outline-none focus-visible:ring-2 focus-visible:ring-drift-primary" onClick={() => void openTransaction(transaction.id)}>View</button></Td>
                    </tr>
                  ))}
                </tbody>
              </table>
              <div className="px-4 py-3 text-xs font-semibold text-drift-text-secondary">Showing {rows.transactions.length} of {rows.total}</div>
            </Card>
          )}
        </div>
        ...
        {detail ? (
          <DetailRail
            ...
            actions={<Badge tone={statusTone(detail.status)}>{label(detail.status)}</Badge>}
          >
            <DefinitionList
              rows={[
                { label: "Owner", value: detail.owner.name },
                { label: "Amount", value: money(detail.amountMinor, detail.currency) },
                { label: "Audience", value: label(detail.invoice.plan.audience) },
                { label: "Invoice", value: <Badge tone={statusTone(detail.invoice.status)}>{label(detail.invoice.status)}</Badge> },
                { label: "Period", value: `${dateLabel(detail.invoice.periodStart)} - ${dateLabel(detail.invoice.periodEnd)}` },
                { label: "Paid at", value: dateTimeLabel(detail.invoice.paidAt) },
                { label: "Method", value: detail.paymentMethodLabel },
                { label: "Created", value: dateTimeLabel(detail.createdAt) },
              ]}
            />
            {detail.failureReason && <div className="mt-5 rounded-xl bg-drift-error-surface px-3 py-2 text-sm font-semibold text-drift-error">{detail.failureReason}</div>}
            <div className="mt-6">
              <Button variant="destructive" icon="undo" disabled={busy || detail.status !== "SUCCEEDED"} onClick={() => void refund(detail)}>{busy ? "Recording..." : "Record refund"}</Button>
            </div>
          </DetailRail>
        ) : (
          <Card><EmptyState message="Select a transaction to inspect invoice, owner, and payment details." /></Card>
        )}
      </div>
    </div>
  );
