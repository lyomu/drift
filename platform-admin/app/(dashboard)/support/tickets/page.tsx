...
export default function SupportTicketsPage() {
  ...
  return (
    <div>
      <PageHeader title="Support Tickets" description="Internal support queue for player, billing, club, match, and technical issues." />
      <ErrorBanner message={error} />

      <div className="mb-4 grid gap-3 md:grid-cols-3">
        <Card className="p-4"><div className="text-sm font-semibold text-drift-text-secondary">Open</div><div className="mt-1 font-display text-3xl font-bold tabular-nums text-drift-text-primary">{counts.open}</div></Card>
        <Card className="p-4"><div className="text-sm font-semibold text-drift-text-secondary">Assigned</div><div className="mt-1 font-display text-3xl font-bold tabular-nums text-drift-text-primary">{counts.assigned}</div></Card>
        <Card className="p-4"><div className="text-sm font-semibold text-drift-text-secondary">Resolved</div><div className="mt-1 font-display text-3xl font-bold tabular-nums text-drift-text-primary">{counts.resolved}</div></Card>
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_440px]">
          ...
          {tickets && tickets.length > 0 && (
            <Card className="overflow-x-auto p-0">
              <table className="w-full min-w-[980px]">
                <thead><tr><Th>Ticket</Th><Th>User</Th><Th>Status</Th><Th>Assignee</Th><Th>Updated</Th><Th className="text-right">Action</Th></tr></thead>
                <tbody>
                  {tickets.map((ticket) => (
                    <tr key={ticket.id} className={selected?.id === ticket.id ? "bg-drift-primary-light/60" : ""}>
                      <Td><div className="font-semibold">{ticket.subject}</div><div className="text-xs text-drift-text-secondary">{label(ticket.category)} / {label(ticket.priority)}</div></Td>
                      <Td>{ticket.user ? <Link href={`/users?query=${encodeURIComponent(ticket.user.email ?? ticket.user.id)}`} className="font-semibold text-drift-primary hover:underline">{personName(ticket.user)}</Link> : <span className="text-drift-text-secondary">Unlinked</span>}</Td>
                      <Td><Badge tone={statusTone(ticket.status)}>{label(ticket.status)}</Badge></Td>
                      <Td>{personName(ticket.assignedTo)}</Td>
                      <Td>{dateTime(ticket.updatedAt)}</Td>
                      <Td className="text-right"><button className="font-semibold text-drift-primary hover:underline focus:outline-none focus-visible:ring-2 focus-visible:ring-drift-primary" onClick={() => setSelectedId(ticket.id)}>Open</button></Td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="space-y-5">
          <Card>
            <h2 className="font-display text-xl font-semibold text-drift-text-primary">Open ticket</h2>
            <form onSubmit={createTicket} className="mt-4 flex flex-col gap-4">
          ...
          {selected && (
            <Card>
              <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
                <div>
                  <h2 className="font-display text-xl font-semibold text-drift-text-primary">{selected.subject}</h2>
                  <p className="mt-1 text-sm text-drift-text-secondary">{dateTime(selected.createdAt)} / {personName(selected.user)}</p>
                </div>
                <Badge tone={statusTone(selected.status)}>{label(selected.status)}</Badge>
              </div>
              <p className="whitespace-pre-wrap rounded-md border border-drift-border bg-drift-background p-3 text-sm leading-6 text-drift-text-primary">{selected.body}</p>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_auto]">
                ...
                {selected.status !== "RESOLVED" && <div className="flex items-end"><Button variant="destructive" disabled={busy} onClick={() => void close(selected)}>Close</Button></div>}
              </div>

              <div className="mt-5">
                <h3 className="text-[13px] font-semibold text-drift-text-secondary">Responses</h3>
                {selected.messages.length === 0 && <div className="mt-2 rounded-md border border-dashed border-drift-border px-4 py-6 text-sm text-drift-text-secondary">No responses yet.</div>}
                <div className="mt-2 space-y-3">
                  {selected.messages.map((message) => (
                    <div key={message.id} className="rounded-md border border-drift-border px-3 py-2">
                      <div className="text-xs font-semibold text-drift-text-secondary">{personName(message.actor)} / {dateTime(message.createdAt)}</div>
              ...
              {selected.resolutionNote && <div className="mt-4 rounded-md border border-drift-success/30 bg-drift-success-surface px-4 py-3 text-sm text-drift-success">Resolution: {selected.resolutionNote}</div>}

              {selected.status !== "RESOLVED" && (
                <div className="mt-5">
                  <Field label="Response"><Textarea rows={4} value={reply} onChange={(event) => setReply(event.target.value)} /></Field>
                  <Button className="mt-3" disabled={busy || !reply.trim()} onClick={() => void respond(selected)}>{busy ? "Saving..." : "Respond"}</Button>
                </div>
              )}
            </Card>
          )}
