...
export default function IntegrationsPage() {
  ...
  return (
    <div>
      <PageHeader title="API / Integration Settings" description="Provider configuration, credential references, and recorded connection health." action={<Button variant="secondary" onClick={startNew}>New integration</Button>} />
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_440px]">
        ...
        <Card>
          <div className="mb-5 flex items-start justify-between gap-3">
            <div><h2 className="font-display text-xl font-semibold text-drift-text-primary">{editing ? "Edit integration" : "Create integration"}</h2><p className="mt-1 text-sm text-drift-text-secondary">Store provider config and references only. Raw credentials belong in the external secret manager.</p></div>
          ...
          <form onSubmit={save} className="flex flex-col gap-4">
            ...
            <Button type="submit" disabled={busy}>{busy ? "Saving..." : editing ? "Save integration" : "Create integration"}</Button>
          </form>
        </Card>
      </div>
    </div>
