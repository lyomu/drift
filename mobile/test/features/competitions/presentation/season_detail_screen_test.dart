  ...
  group('SeasonDetailScreen', () {
    ...
    for (final brightness in Brightness.values) {
      ...
      testWidgets('explains a closed season in $label', (tester) async {
        await pumpScreen(
          ...
          const SeasonDetailScreen(seasonId: 'season-1'),
          brightness: brightness,
          overrides: await overrides(seasonDetail(state: SeasonState.active), null),
        );

        expect(find.text('Registration is closed for this season.'),
            findsOneWidget);
      });
    }
    ...
    testWidgets('shows a spinner while loading', (tester) async {
      await pumpScreen(
        ...
        settle: false,
        overrides: [
          seasonDetailProvider('season-1').overrideWith(
            (ref) => pending<SeasonDetail>(),
          ),
          currentRoundProvider('season-1').overrideWith(
            (ref) => pending<CompetitionRound?>(),
          ),
        ],
      );
    ...
    testWidgets('survives an error without throwing', (tester) async {
      await pumpScreen(
        ...
        overrides: [
          seasonDetailProvider('season-1').overrideWith(
            (ref) => failing<SeasonDetail>(),
          ),
          currentRoundProvider('season-1').overrideWith(
            (ref) => failing<CompetitionRound?>(),
          ),
        ],
      );
