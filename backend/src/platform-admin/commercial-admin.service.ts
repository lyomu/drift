...
export class CommercialAdminService {
  ...
  async listPlans(query: { audience?: string; status?: string }) {
    ...
    return {
      plans: plans.map((plan) => ({
        ...plan,
        subscriptionCounts: usage.get(plan.id) ?? {},
      })),
    };
  }
  ...
  async updatePlan(actorId: string, planId: string, dto: UpsertPaymentPlanDto) {
    ...
    if (
      ...
      (existing.currency !== dto.currency.toUpperCase() ||
        existing.audience !== dto.audience)
    ) {
      throw new BadRequestException(
        'Plans with active subscriptions cannot change audience or currency.',
      );
    }
  ...
  async listPayments(query: {
    ...
    const [transactions, total, totalsByStatus] =
    ...
    for (const totalRow of totalsByStatus) {
      ...
      const transactionCount =
        typeof totalRow._count === 'number'
          ? totalRow._count
          : totalRow._count && totalRow._count !== true
            ? (totalRow._count._all ?? 0)
            : 0;
      row.transactions += transactionCount;
      if (totalRow.status === PaymentTransactionStatus.SUCCEEDED)
        row.collectedMinor += totalRow._sum?.amountMinor ?? 0;
      if (totalRow.status === PaymentTransactionStatus.REFUNDED)
        row.refundedMinor += totalRow._sum?.amountMinor ?? 0;
      if (totalRow.status === PaymentTransactionStatus.FAILED)
        row.failedMinor += totalRow._sum?.amountMinor ?? 0;
      totals.set(totalRow.currency, row);
    }
    return {
      total,
      totalsByCurrency: [...totals.entries()].map(([currency, row]) => ({
        currency,
        ...row,
      })),
  ...
  async listPromotions(query: { status?: string; audience?: string }) {
    ...
    return {
      promotions: promotions.map((promotion) => ({
        ...promotion,
        state: this.promotionState(promotion, now),
      })),
    };
  }
  ...
  async createPromotion(actorId: string, dto: UpsertPromotionDto) {
    ...
    return {
      promotion: { ...promotion, state: this.promotionState(promotion) },
    };
  }
  ...
  async deactivatePromotion(
    ...
    await this.audit.record(
      actorId,
      'commercial.promotion.deactivate',
      'Promotion',
      promotionId,
      {
        code: promotion.code,
  ...
  async listSponsorPlacements(query: {
    state?: string;
    placementKey?: string;
  }) {
    const placements = await this.prisma.sponsorPlacement.findMany({
      where: {
        ...(query.placementKey
          ? {
              placementKey: {
                contains: query.placementKey.trim(),
                mode: 'insensitive',
              },
            }
          : {}),
    ...
    return {
      placements: placements
        .map((placement) => ({
          ...placement,
          state: this.sponsorState(placement, now),
        }))
        .filter((placement) => !query.state || placement.state === query.state),
    };
  }

  async createSponsorPlacement(
    actorId: string,
    dto: UpsertSponsorPlacementDto,
  ) {
    const placement = await this.prisma.sponsorPlacement.create({
  ...
  async updateSponsorPlacement(
    actorId: string,
    placementId: string,
    dto: UpsertSponsorPlacementDto,
  ) {
    const existing = await this.prisma.sponsorPlacement.findUnique({
  ...
  async deactivateSponsorPlacement(
    actorId: string,
    placementId: string,
    dto: DeactivateSponsorPlacementDto,
  ) {
    const existing = await this.prisma.sponsorPlacement.findUnique({
      where: { id: placementId },
    });
  ...
  private promotionData(dto: UpsertPromotionDto) {
    ...
    if (dto.discountType === PromotionDiscountType.PERCENT && !dto.percentOff) {
      throw new BadRequestException(
        'Percent promotions require a percent value.',
      );
    }
    if (
      dto.discountType === PromotionDiscountType.AMOUNT &&
      (!dto.amountOffMinor || !dto.currency)
    ) {
      throw new BadRequestException(
        'Amount promotions require an amount and currency.',
      );
    }
    return {
      code: dto.code.trim().toUpperCase(),
      ...
      percentOff:
        dto.discountType === PromotionDiscountType.PERCENT
          ? (dto.percentOff ?? null)
          : null,
      amountOffMinor:
        dto.discountType === PromotionDiscountType.AMOUNT
          ? (dto.amountOffMinor ?? null)
          : null,
      currency:
        dto.discountType === PromotionDiscountType.AMOUNT
  ...
  private sponsorData(dto: UpsertSponsorPlacementDto) {
    ...
    if (endsAt && endsAt <= startsAt) {
      throw new BadRequestException(
        'Placement end date must be after the start date.',
      );
    }
    return {
      name: dto.name.trim(),
  ...
  private promotionState(
    promotion: { isActive: boolean; endsAt: Date | null },
    now = new Date(),
  ) {
    return promotion.isActive && (!promotion.endsAt || promotion.endsAt > now)
      ? 'ACTIVE'
      : 'EXPIRED';
  }

  private sponsorState(
    placement: { isActive: boolean; startsAt: Date; endsAt: Date | null },
    now = new Date(),
  ) {
    if (!placement.isActive || (placement.endsAt && placement.endsAt <= now))
      return 'ENDED';
  ...
  private transactionDto(
    transaction: Prisma.PaymentTransactionGetPayload<{
    ...
    const owner = transaction.billingAccount.user
      ? {
          ...
          name:
            [
              transaction.billingAccount.user.firstName,
              transaction.billingAccount.user.lastName,
            ]
              .filter(Boolean)
              .join(' ') || transaction.billingAccount.user.email,
