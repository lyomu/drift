...
describe('Competitions Expansion (e2e)', () => {
  ...
  async function onboard(user: TestUser, firstName: string) {
    ...
    await authed(user.token, 'patch', '/users/me/level').send({ userSelectedLevel: 4.0 });
    await authed(user.token, 'patch', '/users/me/goals').send({ goals: ['play_more'] });
    ...
    await authed(user.token, 'patch', '/users/me/padel-interest').send({ padelInterest: 'NO' });
  }
   ...
   */
  async function playMatch(matchId: string, submitter: TestUser, opponent: TestUser, aGames: number, bGames: number) {
    const proposed = await authed(submitter.token, 'post', `/matches/${matchId}/proposals`)
      .send({ options: [new Date(Date.now() + 3_600_000).toISOString()] })
      .expect(201);
    await authed(opponent.token, 'patch', `/matches/${matchId}/proposals/accept`)
      .send({ optionId: proposed.body.latestProposal.options[0].id })
      .expect(200);

    await authed(submitter.token, 'post', `/matches/${matchId}/results`)
      .send({ outcome: 'SCORE', sets: [{ sideAGames: aGames, sideBGames: bGames }] })
      .expect(201);
  ...
  beforeAll(async () => {
    ...
    if (!clubId) {
      const found = await prisma.club.findFirst({ where: { name: `W6 Club ${stamp}` } });
      clubId = found!.id;
    }
  }, 180_000);
  ...
  afterAll(async () => {
    ...
    for (const email of emails) {
      ...
      if (!record) continue;
      const profile = await prisma.tennisProfile.findUnique({ where: { userId: record.id } });
      if (profile) {
        await prisma.availabilitySlot.deleteMany({ where: { tennisProfileId: profile.id } });
        await prisma.assessmentAnswer.deleteMany({ where: { session: { tennisProfileId: profile.id } } });
        await prisma.assessmentSession.deleteMany({ where: { tennisProfileId: profile.id } });
      }
      const matchIds = await prisma.matchParticipant.findMany({
        where: { userId: record.id },
        select: { matchId: true },
      });
      await prisma.match.deleteMany({ where: { id: { in: matchIds.map((m) => m.matchId) } } });
      ...
      await prisma.user.delete({ where: { id: record.id } }).catch(() => undefined);
    }
    await app.close();
  }, 60_000);
  ...
  it('runs a 4-player knockout from creation to COMPLETED', async () => {
    ...
    const created = await authed(olivia.token, 'post', `/clubs/${clubId}/tournaments`)
      .send({
    ...
    await authed(olivia.token, 'post', `/tournaments/${tournamentId}/entries`).expect(201);
    await authed(ben.token, 'post', `/tournaments/${tournamentId}/entries`).expect(201);
    await authed(cara.token, 'post', `/tournaments/${tournamentId}/entries`).expect(201);
    ...
    const drawn = await authed(olivia.token, 'post', `/clubs/${clubId}/tournaments/${tournamentId}/generate-draw`)
      .send({})
      .expect(201);
    ...
    const live = semis.filter((f: any) => f.matchId);
    expect(live).toHaveLength(1);

    // Play the semi: sideA wins, opponent confirms.
    const semi = live[0];
    const submitter =
      semi.sideAUserId === users.olivia.id ? olivia : semi.sideAUserId === users.ben.id ? ben : cara;
    const other =
      submitter === olivia ? (semi.sideBUserId === users.ben.id ? ben : cara) : olivia;
    ...
    const after = await authed(olivia.token, 'get', `/tournaments/${tournamentId}`).expect(200);
    ...
    const fB = finalistB ? Object.values(users).find((u) => u.id === finalistB) : null;
    ...
    const done = await authed(olivia.token, 'get', `/tournaments/${tournamentId}`).expect(200);
  ...
  it('runs a ladder challenge to a rung swap', async () => {
    ...
    const ladder = await authed(olivia.token, 'post', `/clubs/${clubId}/ladders`)
      .send({ name: `W6 Ladder ${stamp}`, challengeRange: 2 })
      .expect(201);
    ladderId = ladder.body.id;

    await authed(olivia.token, 'post', `/ladders/${ladderId}/entries`).expect(201);
    await authed(ben.token, 'post', `/ladders/${ladderId}/entries`).expect(201);
    await authed(cara.token, 'post', `/ladders/${ladderId}/entries`).expect(201);

    const detail = await authed(cara.token, 'get', `/ladders/${ladderId}`).expect(200);
    const positions = detail.body.ladder.entries.map((e: any) => e.position);
    ...
    const challenge = await authed(cara.token, 'post', `/ladders/${ladderId}/challenges`)
      .send({ defenderUserId: users.olivia.id })
      .expect(201);
    expect(challenge.body.state).toBe('PENDING');

    // Only the defender accepts.
    await authed(cara.token, 'post', `/ladders/challenges/${challenge.body.id}/accept`).expect(400);
    await authed(olivia.token, 'post', `/ladders/challenges/${challenge.body.id}/accept`).expect(201);

    const accepted = await prisma.ladderChallenge.findUnique({ where: { id: challenge.body.id } });
    ...
    const after = await authed(cara.token, 'get', `/ladders/${ladderId}`).expect(200);
    const caraPos = after.body.ladder.entries.find((e: any) => e.userId === users.cara.id).position;
    const oliviaPos = after.body.ladder.entries.find((e: any) => e.userId === users.olivia.id).position;
  ...
  it('rejects out-of-range ladder challenges', async () => {
    ...
    const res = await authed(users.cara.token, 'post', `/ladders/${ladderId}/challenges`)
