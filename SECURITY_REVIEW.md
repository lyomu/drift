...
| A07 Identification and Authentication Failures | Partially controlled | Access/refresh token separation, hashed refresh tokens, rotation/revocation, rate limiting, and current-user status checks are present. Mandatory MFA for Platform Admin is now in place (single-use 2FA challenge before the staff JWT). Breached-password screening is now enforced on password set/change through HIBP k-anonymity and fails open so third-party outages cannot break signup/reset. **Owner decision 2026-09-03:** minimum length stays 8 and no character-composition rules are added; existing passwords keep working until changed. Still open: the review's recommendation to strengthen the minimum length beyond eight characters. |
